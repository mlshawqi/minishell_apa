...
// test1.c
t_separation			*ft_new_token(char *str, char *str_backup, int type,
							int status);
...
void					clear_token_list(t_separation **list,
							void (*del)(void *));
...
void					insert_var_value(char *new_str, char *var_value,
							int *pos);
...
int						initialize_default_args(t_separation **token_node,
							t_cmd *cmd);
char					**extend_args_with_tokens(int existing_len,
							char **new_args, t_cmd *cmd,
							t_separation **token_node);
int						append_default_args(t_separation **token_node,
							t_cmd *cmd);
int						process_command_args(t_separation **token_node,
							t_cmd *last_cmd);
char					*combine_vars(t_separation **token_node);
int						count_arguments(t_separation *current_token);
char					**copy_args_to_new_tab(int existing_len,
							char **new_args, t_cmd *last_cmd,
							t_separation *current_token);
void					remove_empty_variables(t_separation **tokens);
int						initialize_echo_arguments(t_separation **token_node,
							t_cmd *last_cmd);
int						append_echo_arguments(t_separation **token_node,
							t_cmd *last_cmd);
void					free_in_out(t_in_out_fds **io);
void					free_string_array(char **array);
int						open_file(t_in_out_fds *io, char *file,
							char *original_filename);
int						handle_append_redirection(t_cmd **last_cmd,
...
int						write_heredoc_input(t_data *data, t_in_out_fds *io,
							int fd);
...
int						process_heredoc(t_data *data, t_cmd **last_cmd,
							t_separation **token_lst);
...
int						handle_input_redirection(t_cmd **last_cmd,
							t_separation **token_lst);
...
void					split_variable_command(t_cmd *last_cmd,
							const char *cmd_str);
...
void					display_token_type(t_separation *token,
							const char *prefix);
...
int						export_cmd(t_data *data, t_env **envrmnt,
							t_env **export, char **args);
...
void					link_node_redirection(t_in_out_fds **head,
							t_in_out_fds *new_node);
...
int						handle_child_process(t_data *data, t_cmd *cmd,
							int **pipes, int i);
